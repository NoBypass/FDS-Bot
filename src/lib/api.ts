...
const graphqlCall = async <T> (query: string, schema: string): Promise<Partial<T>> => {
...
export const api = {
    ...
    add: {
        mojangAccount: async (name: string, schema: string) => {
            return await graphqlCall<MojangAccount>(`addMojangAccount(name: ${name})`, schema)
        },
        hypixelPlayer: async (schema: string) => {
            return await graphqlCall<HypixelPlayer>('addHypixelPlayer(isTracked: false)', schema)
        }
    },
    connect: {
        mojangAccountWithHypixelPlayer: async ({
            mojangAccountId,
            hypixelPlayerId
        }: MojangAccountWithHypixelPlayer, schema: string) => {
...
export const login = async (name: string, password: string): Promise<JwtResponse> => {
    const loginAttempt = await postCall(
        JSON.stringify({ name, password }),
        'login'
    )
    return loginAttempt.jwt != null ? loginAttempt : await postCall(
        JSON.stringify({ name, password }),
        'register'
    ) 
}